...
@views function fg_object(x::AbstractArray{<:AbstractFloat, 3}, g, observations, reconstruction, patches)
    ...
    for dd=1:ndatasets
        ...
        Threads.@threads :static for t=1:observation.nepochs
            ...
            for n=1:observation.nsubaps
                ...
                reconstruction.gradient_object(helpers.g_threads_obj[:, :, :, tid], r[:, :, tid], ω[:, :, tid], helpers.Î_big[:, :, tid], psfs[:, :, :, n, t, :], observation.entropy[n, t], patches.w, patches.npatches, reconstruction.Δλ, reconstruction.nλ)
            end
        end
    end
...
@views function gradient_object_gaussiannoise!(g::AbstractArray{<:AbstractFloat, 3}, r, ω, image_big, psfs, entropy, patch_weights, npatches, Δλ, nλ)
    r .*= ω
...
@views function gradient_object_mixednoise!(g::AbstractArray{<:AbstractFloat, 3}, r, ω, image_big, psfs, entropy, patch_weights, npatches, Δλ, nλ)
...
@views function fg_opd(x, g, observations, atmosphere, masks, patches, reconstruction)
    ## Optimized but unreadable
    FTYPE = gettype(reconstruction)
    ndatasets = length(observations)
    helpers = reconstruction.helpers
    ...
    fill!(helpers.ϵ_threads, zero(FTYPE))
    update_layer_figure(x, atmosphere, reconstruction)
    ...
    for dd=1:ndatasets
        ...
        Threads.@threads :static for t=1:observation.nepochs
            ...
            for n=1:observation.nsubaps
                ...
                for np=1:patches.npatches
                    for w₁=1:reconstruction.nλ
                        for w₂=1:reconstruction.nλint 
                            ...
                            pupil2psf!(Î_big, helpers.containers_builddim_real[:, :, tid], mask.masks[:, :, n, w], P[:, :, np, w], p[:, :, np, w], A[:, :, np, n, w], ϕ_composite[:, :, np, w], α, scale_psf[w], helpers.ift[tid], refraction[w])
                ...
                ω[:, :, tid] .= reconstruction.weight_function(observation.entropy[n, t], observation.model_images[:, :, n, t], observation.detector.rn)
                helpers.ϵ_threads[tid] += loglikelihood_gaussian(r[:, :, tid], observation.images[:, :, n, t], observation.model_images[:, :, n, t], ω[:, :, tid])
                reconstruction.gradient_opd(helpers.g_threads_opd[:, :, :, tid], r[:, :, tid], ω[:, :, tid], P, p, helpers.c[:, :, tid], helpers.d[:, :, tid], helpers.d2[:, :, tid], reconstruction.λtotal, reconstruction.Δλtotal, reconstruction.nλ, reconstruction.nλint, α, atmosphere.nlayers, helpers.o_corr[:, :, tid], observation.entropy[n, t], patches.npatches, reconstruction.smoothing, helpers.k_corr[tid], refraction_adj, extractor_adj, helpers.ift[tid], helpers.containers_builddim_real[:, :, tid], helpers.containers_sdim_real[:, :, tid])
            end
        end
    end
...
@views function gradient_opd_gaussiannoise!(g, r, ω, P, p, c, d, d2, λ, Δλ, nλ, nλint, α, nlayers, o_corr, entropy, npatches, smoothing, k_corr, refraction_adj, extractor_adj, ifft_prealloc, container_builddim_real, container_sdim_real)
    FTYPE = eltype(r)
    r .*= ω
    block_replicate!(c, r)
    conj!(p)
...
@views function gradient_opd_mixednoise!(g, r, ω, P, p, c, d, d2, λ, Δλ, nλ, nλint, α, nlayers, o_corr, entropy, npatches, smoothing, k_corr, refraction_adj, extractor_adj, ifft_prealloc, container_builddim_real, container_sdim_real)
    FTYPE = eltype(r)
    r .= 2 .* ω .* r .- (ω .* r).^2 .* entropy
    block_replicate!(c, r)
    conj!(p)
